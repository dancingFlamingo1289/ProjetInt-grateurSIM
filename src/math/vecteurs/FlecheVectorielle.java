 ...
 * 
 * Notez qu'il est possible de modifier la longueur du trait de tete ainsi que l'angle entre les 
 * traits qui forment la tete.
 * 
 * Il est aussi possible de modifier la longueur du corps de la fleche tout en conservant son 
 * origine, en specifiant un facteur de redimensionnement. Ceci est parfois necessaire
 * quand le module du vecteur est trop grand/trop petit pour donner un resultat visuel interessant.
...
public class FlecheVectorielle implements Dessinable, Serializable {
	...
	/**Variable de type double qui représente la position du début de la flèche.**/
	private double x1, y1;
	/**Variable de type double qui représente la position du fin de la flèche.**/
	private double x2, y2;
	/**geometrie necessaire **/
	private Line2D.Double corps, traitDeTete;   
	/**en degres, angle par defaut entre les deux segments formant la tete de fleche. **/
	private double angleTete = 30;        
	/**longueur par defaut des segments formant la tete. **/
	private double longueurTraitDeTete = 1;  
	/**Variable de type double qui permet de convertir les pixels en mètre. **/
	 ...
	 */
	public FlecheVectorielle (double x1, double y1, Vecteur3D vec) {
		this.x1 = x1;
		this.y1 = y1;
		this.x2 = x1 + vec.getX();
		this.y2 = y1 + vec.getY();
