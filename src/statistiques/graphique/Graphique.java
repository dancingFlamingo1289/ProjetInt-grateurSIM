package statistiques.graphique ;
...
public class Graphique extends JPanel {
	...
	private final double DEFAUT_Y_MAX = 15 ;
	...
	//private final int NB_GRADUATIONS_X, NB_GRADUATION_Y ;
	...
	public void poserUnPointDeDepart(double x, double y) {
		...
		xMin = DEFAUT_X_MIN/ratioAffichageX ;
		xMax = DEFAUT_X_MAX/ratioAffichageX ;
		yMin = DEFAUT_Y_MIN/ratioAffichageY ;
		yMax = DEFAUT_Y_MAX/ratioAffichageY ;
	...
	private Path2D.Double creerLaCourbe() {
		...
		if (pointsADessinerX.size() - 1 != 0 && pointsADessinerY.size() != 0 && courbe != null) {
			courbe.moveTo(pointsADessinerX.get(0), 
					pointsADessinerY.get(0)) ;
	...
	private void actualiserLesGraduations(double x, double y) {
		recadrer(xMin, xMax, yMin, Math.max(y, yMax)) ;
	...
	private void ajouterUneGraduation() {
		recadrer(xMin + ratioAffichageX, xMax + ratioAffichageX, yMin, yMax + ratioAffichageY) ;
	}
	...
	private void dessinerTaquetsGrillageX(Graphics2D g2d) {
		...
		for (double i = xMin ; i <= xMax ; i += 1) {
			double posX = (i - xMin) * pixelsParUniteX ;
	...
	private void dessinerTaquetsGrillageY(Graphics2D g2d) {
		...
		for (double j = yMin ; j <= yMax ; j += 1) {
			...
			if (j != 0) { // Pour éviter de cacher l'axes des y.
				...
				// On place une valeur à chaque taquet.
				g2d.setColor(Color.orange) ;
				g2d.drawString(String.format("%.1f", (yMax - j)*ratioAffichageY), 
	...
	public void ajouterUnPointSurLaCourbe(double x, double y) {
		boolean capaciteMaxAtteinte = x >= xMax;
	    if (courbe != null) {
	        pointsADessinerX.add(x) ;
	        pointsADessinerY.add(y) ;

	        if (capaciteMaxAtteinte) {
	            pointsADessinerX.remove(0) ;
	            pointsADessinerY.remove(0) ;
	            poserUnPointDeDepart(pointsADessinerX.get(0), pointsADessinerY.get(0)) ;
	            recadrer(xMin + ratioAffichageX, xMax + ratioAffichageX, yMin, yMax) ;
	            capaciteMaxAtteinte = false ;
	        }

	        // Mettre à jour la courbe avec les nouveaux points
	        courbe = creerLaCourbe() ;

	        actualiserLesGraduations(x, y) ;
	    }
	...
	public String toString() {
		return "Graphique [xMin=" + xMin + ", xMax=" + xMax + ", yMin=" + yMin + ", yMax=" + 
				yMax + ", ratioAffichageX="
				+ ratioAffichageX + ", ratioAffichageY=" + ratioAffichageY + ", ptDepartX=" + 
				ptDepartX + ", ptDepartY="
				+ ptDepartY + ", titreGraphique=" + titreGraphique + "]";
	}
